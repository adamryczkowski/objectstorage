...
#  debugonce(add_runtime_objects_internal)
  modify_objects(storagepath = storagepath, obj.environment = env, objects_to_add = 'a', parallel_cpus = 0)
  env2<-new.env()
...
#    debugonce(double_storage)
  ...
  env$a<-'bli'
  modify_objects(storagepath = storagepath, obj.environment = env, objects_to_add = c('a','b'), parallel_cpus = 0)
  env2<-new.env()
  load_objects(storagepath, objectnames = c('a','b'), target_environment = env2, flag_double_check_digest = TRUE)
  testthat::expect_equivalent(env2, env)
...
test_that("Updating small object in multi-object split archive", {
  ...
  env$b<-'bli'
  #debugonce(modify_runtime_objects)
  modify_objects(storagepath = storagepath, obj.environment = env, objects_to_add = c('b'), parallel_cpus = 0)
  env2<-new.env()
  load_objects(storagepath, objectnames = c('a','b'), target_environment = env2, flag_double_check_digest = TRUE)
  testthat::expect_equivalent(env2, env)
})
